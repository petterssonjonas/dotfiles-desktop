...
run_once({ "picom" })
run_once({ "redshift" })
--run_once({ "xautolock -time 10 -locker '/usr/share/i3lock-fancy/lock -f PragmataPro -t Locked' -- scrot" })
...
local dmenu_settings = "dmenu_run -fn 'OperatorMonoLigBook-10' -i -l 12 -p 'Run:' -nb '#32302f' -nf '#a89984' -sb '#458588' -sf '#2d2d2d' -h 10 -w 800 -y 350 -x 400"
local rofi_settings = "rofi -show run"
local i3lock_settings = "i3lock-fancy -f OperatorMonoLigBook -t 'Locked' -n -- scrot"
local modkey       = "Mod4"
local altkey       = "Mod1"
...
naughty.config.defaults.font = "OperatorMonoLigBook 12"
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "r", function ()
        awful.spawn(string.format("dmenu_run -i -fn 'PragmataPro' -nb '%s' -nf '%s' -sb '%s' -sf '%s'",
...
awful.rules.rules = {
   ...
   -- Set Firefox to always map on the first tag on screen 1.
    ...
    { rule = { class = "mpv" },
      properties = { maximized = true } },    { rule = { class = "Nm-connection-editor" },
    { rule = { class = "Tilda"},
